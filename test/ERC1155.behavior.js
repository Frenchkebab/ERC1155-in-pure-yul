  ...
  describe('like an ERC1155', function () {
    describe('uri', async function () {
      it('should return uri string with id input substituted', async function () {
        ...
        const ID1 = 1;
        const ID2 = 1234;
        expect(await this.token.uri(ethers.BigNumber.from(ID0))).to.equal(
          `https://token-cdn-domain/${ID0}.json`
        );
        expect(await this.token.uri(ethers.BigNumber.from(ID1))).to.equal(
          `https://token-cdn-domain/${ID1}.json`
        );
        expect(await this.token.uri(ethers.BigNumber.from(ID2))).to.equal(
